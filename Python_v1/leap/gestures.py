...
class MyListener(leap.Listener):
    ...
    def handle_swiping(self):
        ...
        if self.swipe_direction == None:
            ...
            if self.hand.palm.position.x > self.forward_swipe_threshold and self.handle_static_gesture("pinching"):
                self.swipe_direction = "forward"
                self.starting_x = self.hand.palm.position.x
            elif self.hand.palm.position.x < self.back_swipe_threshold and self.handle_static_gesture("pinching"):
            ...
            if not self.handle_static_gesture("pinching"):
                self.swipe_direction = None
                print("stopped")
            else:
                print("swiping")
                # translate gesture to keypress
                # if self.swipe_direction == "forward" and self.hand.palm.position.x < self.back_swipe_threshold:
                ...
                #     self.swipe_direction = None
                # elif self.swipe_direction == "back" and self.hand.palm.position.x > self.forward_swipe_threshold:
                #     self.ending_x = self.hand.palm.position.x
                #     print(f"Gesture: swipe | Hand: {self.hand_type} | Direction: {self.swipe_direction} | Position: {self.starting_x} to {self.ending_x}")
